...
type Cron struct {
	idGen    *ytype.Int64    // Used for unique name generation.
	status   *ytype.Int      // Timed task status(0: Not Start; 1: Running; 2: Stopped; -1: Closed)
	entries  *ymap.StrAnyMap // All timed task entries.
	logPath  *ytype.String   // Logging path(folder).
	logLevel *ytype.Int      // Logging level.
}

// New returns a new Cron object with default settings.
func New() *Cron {
	return &Cron{
		idGen:    ytype.NewInt64(),
		status:   ytype.NewInt(StatusRunning),
		entries:  ymap.NewStrAnyMap(true),
		logPath:  ytype.NewString(),
		logLevel: ytype.NewInt(ylog.LEVEL_PROD),
	}
}
...
func (c *Cron) Entries() []*Entry {
	array := yarray.NewSortedArraySize(c.entries.Size(), func(v1, v2 interface{}) int {
		entry1 := v1.(*Entry)
		entry2 := v2.(*Entry)
