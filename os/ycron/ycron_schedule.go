...
const (
	...
	yregex_FOR_CRON = `^([\-/\d\*\?,]+)\s+([\-/\d\*\?,]+)\s+([\-/\d\*\?,]+)\s+([\-/\d\*\?,]+)\s+([\-/\d\*\?,A-Za-z]+)\s+([\-/\d\*\?,A-Za-z]+)$`
)
...
func newSchedule(pattern string) (*cronSchedule, error) {
	...
	if match, _ := yregex.MatchString(`(@\w+)\s*(\w*)\s*`, pattern); len(match) > 0 {
		key := strings.ToLower(match[1])
		if v, ok := predefinedPatternMap[key]; ok {
			pattern = v
		} else if strings.Compare(key, "@every") == 0 {
			if d, err := ytime.ParseDuration(match[2]); err != nil {
				return nil, err
			} else {
				return &cronSchedule{
	...
	// 0 0 0 1 1 2
	if match, _ := yregex.MatchString(yregex_FOR_CRON, pattern); len(match) == 7 {
...
func parseItemValue(value string, fieldType byte) (int, error) {
	if yregex.IsMatchString(`^\d+$`, value) {
		// Pure number.
