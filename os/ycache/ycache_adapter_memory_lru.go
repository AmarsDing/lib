...
type adapterMemoryLru struct {
	...
	data    *ymap.Map      // Key mapping to the item of the list.
	list    *ylist.List    // Key list.
	rawList *ylist.List    // History for key adding.
	closed  *ytype.Bool    // Closed or not.
}
...
func newMemCacheLru(cache *adapterMemory) *adapterMemoryLru {
	lru := &adapterMemoryLru{
		...
		data:    ymap.New(true),
		list:    ylist.New(true),
		rawList: ylist.New(true),
		closed:  ytype.NewBool(),
	}
	ytimer.AddSingleton(time.Second, lru.SyncAndClear)
	return lru
}
...
func (lru *adapterMemoryLru) Remove(key interface{}) {
	if v := lru.data.Get(key); v != nil {
		lru.data.Remove(key)
		lru.list.Remove(v.(*ylist.Element))
	}
}
