...
type adapterMemory struct {
	...
	lruGetList  *ylist.List               // lruGetList is the LRU history according with Get function.
	eventList   *ylist.List               // eventList is the asynchronous event list for internal data synchronization.
	closed      *ytype.Bool               // closed controls the cache closed or not.
}
...
func (c *adapterMemory) getInternalExpire(duration time.Duration) int64 {
	if duration == 0 {
		...
		return ytime.TimestampMilli() + duration.Nanoseconds()/1000000
	}
}
...
func (c *adapterMemory) syncEventAndClearExpired() {
	...
	var (
		expireSet *yset.Set
		ek        = c.makeExpireKey(ytime.TimestampMilli())
