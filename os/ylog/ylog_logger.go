...
func (l *Logger) getFilePath(now time.Time) string {
	// Content containing "{}" in the file name is formatted using ytime.
	file, _ := yregex.ReplaceStringFunc(`{.+?}`, l.config.File, func(s string) string {
		return ytime.New(now).Format(strings.Trim(s, "{}"))
	})
	file = yfile.Join(l.config.Path, file)
	return file
}
...
func (l *Logger) print(ctx context.Context, level int, values ...interface{}) {
	...
	if !p.init.Val() && p.init.Cas(false, true) {
		...
		if p.config.RotateSize > 0 || p.config.RotateExpire > 0 {
			ytimer.AddOnce(p.config.RotateCheckInterval, p.rotateChecksTimely)
	...
	if l.config.HeaderPrint {
		...
		if l.config.Flags&(F_FILE_LONG|F_FILE_SHORT|F_CALLER_FN) > 0 {
			callerFnName, path, line := ydebug.CallerWithFilter(pathFilterKey, l.config.StSkip)
			...
			if l.config.Flags&F_FILE_SHORT > 0 {
				input.CallerPath = fmt.Sprintf(`%s:%d:`, yfile.Basename(path), line)
			}
		}
		// Prefix.
...
func (l *Logger) printToFile(t time.Time, buffer *bytes.Buffer) {
	var (
		loyfilePath   = l.getFilePath(t)
		memoryLockKey = "glog.printToFile:" + loyfilePath
	)
	ymlock.Lock(memoryLockKey)
	defer ymlock.Unlock(memoryLockKey)

	// Rotation file size checks.
	if l.config.RotateSize > 0 {
		if yfile.Size(loyfilePath) > l.config.RotateSize {
			l.rotateFileBySize(t)
		}
	}
	// Logging content outputting to disk file.
	if file := l.getFilePointer(loyfilePath); file == nil {
		intlog.Errorf(`got nil file pointer for: %s`, loyfilePath)
	} else {
...
func (l *Logger) getFilePointer(path string) *yfpool.File {
	file, err := yfpool.Open(
		path,
		defaultFileFlags,
		defaultFilePerm,
		defaultFileExpire,
...
func (l *Logger) GetStack(skip ...int) string {
	...
	return ydebug.StackWithFilters(filters, stackSkip)
}
