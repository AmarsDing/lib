...
func (l *Logger) SetConfigWithMap(m map[string]interface{}) error {
	...
	levelKey, levelValue := yutil.MapPossibleItemByKey(m, "Level")
	if levelValue != nil {
		if level, ok := levelStringMap[strings.ToUpper(yconv.String(levelValue))]; ok {
			m[levelKey] = level
		} else {
	...
	rotateSizeKey, rotateSizeValue := yutil.MapPossibleItemByKey(m, "RotateSize")
	if rotateSizeValue != nil {
		m[rotateSizeKey] = yfile.StrToSize(yconv.String(rotateSizeValue))
		if m[rotateSizeKey] == -1 {
			return errors.New(fmt.Sprintf(`invalid rotate size: %v`, rotateSizeValue))
		}
	}
	err := yconv.Struct(m, &l.config)
	if err != nil {
		return err
	}
...
func (l *Logger) SetPath(path string) error {
	...
	if !yfile.Exists(path) {
		if err := yfile.Mkdir(path); err != nil {
			//fmt.Fprintln(os.Stderr, fmt.Sprintf(`[glog] mkdir "%s" failed: %s`, path, err.Error()))
			return yerror.Wrapf(err, `Mkdir "%s" failed in PWD "%s"`, path, yfile.Pwd())
		}
	}
	l.config.Path = strings.TrimRight(path, yfile.Separator)
	return nil
}
