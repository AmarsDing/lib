...
type SPath struct {
	paths *yarray.StrArray // The searching directories array.
	cache *ymap.StrStrMap  // Searching cache map, it is not enabled if it's nil.
}
...
var (
	...
	pathsMap = ymap.NewStrAnyMap(true)
)

// New creates and returns a new path searching manager.
func New(path string, cache bool) *SPath {
	sp := &SPath{
		paths: yarray.NewStrArray(true),
	}
	if cache {
		sp.cache = ymap.NewStrStrMap(true)
	}
	if len(path) > 0 {
...
func (sp *SPath) Set(path string) (realPath string, err error) {
	...
	if realPath == "" {
		...
		if realPath == "" {
			realPath = yfile.RealPath(yfile.Pwd() + yfile.Separator + path)
		}
	}
	if realPath == "" {
	...
	if yfile.IsDir(realPath) {
		realPath = strings.TrimRight(realPath, yfile.Separator)
	...
	if sp.cache == nil {
		...
		if len(indexFiles) > 0 && isDir {
			...
			for _, file := range indexFiles {
				path = filePath + yfile.Separator + file
