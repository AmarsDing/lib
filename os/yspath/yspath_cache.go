...
func (sp *SPath) nameFromPath(filePath, rootPath string) string {
	name := ystr.Replace(filePath, rootPath, "")
...
func (sp *SPath) addToCache(filePath, rootPath string) {
	...
	if idDir {
		if files, err := yfile.ScanDir(filePath, "*", true); err == nil {
			//fmt.Println("gspath add to cache:", filePath, files)
			for _, path := range files {
				sp.cache.SetIfNotExist(sp.nameFromPath(path, rootPath), sp.makeCacheValue(path, yfile.IsDir(path)))
			}
		} else {
...
// addMonitorByPath adds yfsnotify monitoring recursively.
...
// removeMonitorByPath removes yfsnotify monitoring of <path> recursively.
func (sp *SPath) removeMonitorByPath(path string) {
	if sp.cache == nil {
		return
	}
	_ = yfsnotify.Remove(path)
}
