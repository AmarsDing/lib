...
func newFilePool(p *Pool, path string, flag int, perm os.FileMode, ttl time.Duration) *ypool.Pool {
	pool := ypool.New(ttl, func() (interface{}, error) {
...
func (p *Pool) File() (*File, error) {
	if v, err := p.pool.Get(); err != nil {
		...
		if !p.init.Val() && p.init.Cas(false, true) {
			_, _ = yfsnotify.Add(f.path, func(event *yfsnotify.Event) {
