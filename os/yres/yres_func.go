...
import "github.com/AmarsDing/lib/os/yres"
...
func Pack(srcPaths string, keyPrefix ...string) ([]byte, error) {
	...
	return ycompress.Gzip(buffer.Bytes(), 9)
}
...
func PackToFile(srcPaths, dstPath string, keyPrefix ...string) error {
	...
	return yfile.PutBytes(dstPath, data)
}
...
func PackToGoFile(srcPath, goFilePath, pkgName string, keyPrefix ...string) error {
	...
	return yfile.PutContents(
		goFilePath,
		fmt.Sprintf(ystr.TrimLeft(packedGoSouceTemplate), pkgName, ybase64.EncodeToString(data)),
	)
}

// Unpack unpacks the content specified by <path> to []*File.
func Unpack(path string) ([]*File, error) {
	realPath, err := yfile.Search(path)
	if err != nil {
		return nil, err
	}
	return UnpackContent(yfile.GetContents(realPath))
}
...
func UnpackContent(content string) ([]*File, error) {
	...
	if isHexStr(content) {
		...
		data, err = ycompress.UnGzip(hexStrToBytes(content))
		if err != nil {
			return nil, err
		}
	} else if isBase64(content) {
		// New version packing string using base64.
		b, err := ybase64.DecodeString(content)
		if err != nil {
			return nil, err
		}
		data, err = ycompress.UnGzip(b)
		if err != nil {
			return nil, err
		}
	} else {
		data, err = ycompress.UnGzip(yconv.UnsafeStrToBytes(content))
		if err != nil {
			return nil, err
		}
	}
