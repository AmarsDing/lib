...
import (
	...
	"github.com/AmarsDing/lib/internal/fileinfo"
...
func doZipPathWriter(path string, exclude string, zipWriter *zip.Writer, prefix ...string) error {
	...
	path, err = yfile.Search(path)
	if err != nil {
		return err
	}
	if yfile.IsDir(path) {
		files, err = yfile.ScanDir(path, "*", true)
		if err != nil {
			return err
		}
	} else {
	...
	if len(headerPrefix) > 0 && yfile.IsDir(path) {
		headerPrefix += "/"
	}
	if headerPrefix == "" {
		headerPrefix = yfile.Basename(path)
	}
	...
	for _, file := range files {
		...
		err = zipFile(file, headerPrefix+yfile.Dir(file[len(path):]), zipWriter)
		if err != nil {
			return err
		}
	}
...
func createFileHeader(info os.FileInfo, prefix string) (*zip.FileHeader, error) {
	...
	if len(prefix) > 0 {
		...
		header.Name, _ = yregex.ReplaceString(`/{2,}`, `/`, header.Name)
	}
	return header, nil
}
