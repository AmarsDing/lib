...
import (
	...
	"github.com/AmarsDing/lib/container/ytree"
...
func New() *Resource {
	return &Resource{
		tree: ytree.NewBTree(gDEFAULT_TREE_M, func(v1, v2 interface{}) int {
...
func (r *Resource) Load(path string, prefix ...string) error {
	...
	return r.Add(yfile.GetContents(realPath), prefix...)
}
...
func (r *Resource) doScanDir(path string, pattern string, recursive bool, onlyFile bool) []*File {
	...
	r.tree.IteratorFrom(path, true, func(key, value interface{}) bool {
		...
		for _, p := range patterns {
			if match, err := filepath.Match(p, yfile.Basename(name)); err == nil && match {
...
func (r *Resource) Dump() {
	...
	r.tree.Iterator(func(key, value interface{}) bool {
		...
		fmt.Printf("%v %7s %s\n", ytime.New(info.ModTime()).ISO8601(), yfile.FormatSize(info.Size()), key)
		return true
	})
