...
type Timer struct {
	...
	status  *ytype.Int     // status is the current timer status.
	ticks   *ytype.Int64   // ticks is the proceeded interval number by the timer.
...
var (
	defaultTimer    = New()
	defaultInterval = ycmd.GetOptWithEnv(
