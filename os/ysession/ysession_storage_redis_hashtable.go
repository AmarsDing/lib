...
func NewStorageRedisHashTable(redis *yredis.Redis, prefix ...string) *StorageRedisHashTable {
	if redis == nil {
...
func (s *StorageRedisHashTable) GetMap(id string) map[string]interface{} {
	...
	for i := 0; i < len(array); i += 2 {
		if array[i+1] != nil {
			m[yconv.String(array[i])] = yconv.String(array[i+1])
		} else {
			m[yconv.String(array[i])] = array[i+1]
		}
	}
	return m
}
...
func (s *StorageRedisHashTable) GetSession(id string, ttl time.Duration, data *ymap.StrAnyMap) (*ymap.StrAnyMap, error) {
	...
	if r.Bool() {
		return ymap.NewStrAnyMap(true), nil
	}
	return nil, nil
}
...
func (s *StorageRedisHashTable) SetSession(id string, data *ymap.StrAnyMap, ttl time.Duration) error {
