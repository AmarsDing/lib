...
import (
	...
	"github.com/AmarsDing/lib/crypto/yaes"
...
type StorageFile struct {
	...
	cryptoKey     []byte
	cryptoEnabled bool
	updatingIdSet *yset.StrSet
}

var (
	DefaultStorageFilePath          = yfile.TempDir("gsessions")
...
func NewStorageFile(path ...string) *StorageFile {
	...
	if len(path) > 0 && path[0] != "" {
		storagePath, _ = yfile.Search(path[0])
		if storagePath == "" {
			panic(yerror.Newf("'%s' does not exist", path[0]))
		}
		if !yfile.IsWritable(storagePath) {
			panic(yerror.Newf("'%s' is not writable", path[0]))
		}
	}
	if storagePath != "" {
		if err := yfile.Mkdir(storagePath); err != nil {
			panic(yerror.Wrapf(err, `Mkdir "%s" failed in PWD "%s"`, path, yfile.Pwd()))
		}
	}
	s := &StorageFile{
		path:          storagePath,
		...
		updatingIdSet: yset.NewStrSet(true),
	}

	ytimer.AddSingleton(DefaultStorageFileLoopInterval, s.updateSessionTimely)
	return s
}
...
// GetSession returns the session data as *ymap.StrAnyMap for given session id from storage.
//
...
func (s *StorageFile) GetSession(id string, ttl time.Duration, data *ymap.StrAnyMap) (*ymap.StrAnyMap, error) {
	if data != nil {
		return data, nil
	}
	//intlog.Printf("StorageFile.GetSession: %s, %v", id, ttl)
	path := s.sessionFilePath(id)
	content := yfile.GetBytes(path)
	if len(content) > 8 {
		timestampMilli := ybinary.DecodeToInt64(content[:8])
		if timestampMilli+ttl.Nanoseconds()/1e6 < ytime.TimestampMilli() {
			return nil, nil
		}
		var err error
		content = content[8:]
		// Decrypt with AES.
		if s.cryptoEnabled {
			content, err = yaes.Decrypt(content, DefaultStorageFileCryptoKey)
			if err != nil {
				return nil, err
			}
		}
		...
		return ymap.NewStrAnyMapFrom(m, true), nil
	}
	return nil, nil
}
...
func (s *StorageFile) SetSession(id string, data *ymap.StrAnyMap, ttl time.Duration) error {
	...
	// Encrypt with AES.
	if s.cryptoEnabled {
		content, err = yaes.Encrypt(content, DefaultStorageFileCryptoKey)
		if err != nil {
			return err
		}
	}
	file, err := yfile.OpenWithFlagPerm(
	...
	defer file.Close()
	if _, err = file.Write(ybinary.EncodeInt64(ytime.TimestampMilli())); err != nil {
		return err
	}
...
func (s *StorageFile) updateSessionTTl(id string) error {
	...
	path := s.sessionFilePath(id)
	file, err := yfile.OpenWithFlag(path, os.O_WRONLY)
	if err != nil {
		return err
	}
	if _, err = file.WriteAt(ybinary.EncodeInt64(ytime.TimestampMilli()), 0); err != nil {
		return err
	}
	return file.Close()
}
