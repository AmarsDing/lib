...
import (
	...
	"github.com/AmarsDing/lib/database/yredis"
...
type StorageRedis struct {
	redis         *yredis.Redis   // Redis client for session storage.
	prefix        string          // Redis key prefix for session id.
	updatingIdMap *ymap.StrIntMap // Updating TTL set for session id.
}
...
func NewStorageRedis(redis *yredis.Redis, prefix ...string) *StorageRedis {
	if redis == nil {
	...
	s := &StorageRedis{
		redis:         redis,
		updatingIdMap: ymap.NewStrIntMap(true),
	}
	if len(prefix) > 0 && prefix[0] != "" {
		s.prefix = prefix[0]
	}
	// Batch updates the TTL for session ids timely.
	ytimer.AddSingleton(DefaultStorageRedisLoopInterval, func() {
...
func (s *StorageRedis) GetSession(id string, ttl time.Duration, data *ymap.StrAnyMap) (*ymap.StrAnyMap, error) {
...
func (s *StorageRedis) SetSession(id string, data *ymap.StrAnyMap, ttl time.Duration) error {
