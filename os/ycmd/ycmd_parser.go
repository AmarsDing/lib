...
func ParseWithArgs(args []string, supportedOptions map[string]bool, strict ...bool) (*Parser, error) {
	...
	for i := 0; i < len(args); {
		if option := parser.parseOption(args[i]); option != "" {
			array, _ := yregex.MatchString(`^(.+?)=(.+)$`, option)
			if len(array) == 3 {
...
func (p *Parser) parseOption(argument string) string {
	array, _ := yregex.MatchString(`^\-{1,2}(.+)$`, argument)
	if len(array) == 2 {
		return array[1]
	}
	return ""
...
func (p *Parser) setOptionValue(name, value string) {
	for optionName, _ := range p.passedOptions {
		array := ystr.SplitAndTrim(optionName, ",")
		for _, v := range array {
...
// GetOptVar returns the option value named <name> as yvar.Var.
func (p *Parser) GetOptVar(name string, def ...interface{}) *yvar.Var {
	if p.ContainsOpt(name) {
		return yvar.New(p.GetOpt(name))
	}
	if len(def) > 0 {
		return yvar.New(def[0])
	}
	return yvar.New(nil)
}
...
// GetAryvar returns the argument at <index> as yvar.Var.
func (p *Parser) GetAryvar(index int, def ...string) *yvar.Var {
	return yvar.New(p.GetArg(index, def...))
}
