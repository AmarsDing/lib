...
func StrToTime(str string, format ...string) (*Time, error) {
	...
	if match = timeRegex1.FindStringSubmatch(str); len(match) > 0 && match[1] != "" {
		...
		return nil, yerror.New("unsupported time format")
	}
	...
	if match[6] != "" {
		...
		if len(zone) <= 6 {
			...
			if h > 24 || m > 59 || s > 59 {
				return nil, yerror.Newf("invalid zone string: %s", match[6])
			}
	...
	if month <= 0 || day <= 0 {
		return nil, yerror.New("invalid time string:" + str)
	}
...
func ParseTimeFromContent(content string, format ...string) *Time {
	if len(format) > 0 {
		if match, err := yregex.MatchString(formatToRegexPattern(format[0]), content); err == nil && len(match) > 0 {
...
func ParseDuration(s string) (time.Duration, error) {
	...
	match, err := yregex.MatchString(`^([\-\d]+)[dD](.*)$`, s)
	if err != nil {
		return 0, err
	}
	if len(match) == 3 {
