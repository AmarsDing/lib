// Package ysmtp provides a simple SMTP client to access remote mail server.
//
// Eg:
...
func (s *SMTP) SendMail(from, tos, subject, body string, contentType ...string) error {
	...
	header := map[string]string{
		...
		"Subject":                   fmt.Sprintf("=?UTF-8?B?%s?=", contentEncoding.EncodeToString(yconv.UnsafeStrToBytes(subject))),
		"MIME-Version":              "1.0",
	...
	message += "\r\n" + contentEncoding.EncodeToString(yconv.UnsafeStrToBytes(body))
	return smtp.SendMail(
		address,
		smtp.PlainAuth("", s.Username, s.Password, server),
		from,
		tosArr,
		yconv.UnsafeStrToBytes(message),
	)
}
