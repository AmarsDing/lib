...
type PoolConn struct {
	*Conn              // Underlying connection object.
	pool   *ypool.Pool // Connection pool, which is not a really connection pool, but a connection reusable pool.
...
var (
	...
	addressPoolMap = ymap.NewStrAnyMap(true)
)
...
func NewPoolConn(addr string, timeout ...time.Duration) (*PoolConn, error) {
	v := addressPoolMap.GetOrSetFuncLock(addr, func() interface{} {
		var pool *ypool.Pool
		pool = ypool.New(gDEFAULT_POOL_EXPIRE, func() (interface{}, error) {
	...
	if v, err := v.(*ypool.Pool).Get(); err == nil {
