...
func (v *Validator) CheckValue(value interface{}) Error {
	return v.doCheckValue("", value, yconv.String(v.rules), v.messages, v.data, yconv.Map(v.data))
}
...
func (v *Validator) doCheckValue(
	...
	default:
		for k, v := range yconv.Map(messages) {
			customMsgMap[k] = yconv.String(v)
		}
	}
...
func (v *Validator) doCheckBuildInRules(
	...
	case "same":
		...
		if foundValue != nil {
			if strings.Compare(valueStr, yconv.String(foundValue)) == 0 {
				match = true
			}
		}
		if !match {
