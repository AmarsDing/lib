...
var (
	sequence      ytype.Uint32                             // Sequence for unique purpose of current process.
...
func init() {
	...
	if len(macs) > 0 {
		...
		s := strconv.FormatUint(uint64(yhash.DJBHash(macAddrBytes)), 36)
		copy(b, s)
		macAddrStr = string(b)
	}
...
func getDataHashStr(data []byte) []byte {
	b := []byte{'0', '0', '0', '0', '0', '0', '0'}
	s := strconv.FormatUint(uint64(yhash.DJBHash(data)), 36)
	copy(b, s)
	return b
}
