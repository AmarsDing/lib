...
type Pool struct {
	list    *ylist.List                 // Available/idle items list.
	closed  *ytype.Bool                 // Whether the pool is closed.
...
func New(ttl time.Duration, newFunc NewFunc, expireFunc ...ExpireFunc) *Pool {
	...
	ytimer.AddSingleton(time.Second, r.checkExpireItems)
	return r
}
...
func (p *Pool) Put(value interface{}) error {
	...
	if p.TTL == 0 {
		...
		item.expireAt = ytime.TimestampMilli() + p.TTL.Nanoseconds()/1000000
	}
...
func (p *Pool) Get() (interface{}, error) {
	for !p.closed.Val() {
		if r := p.list.PopFront(); r != nil {
			f := r.(*poolItem)
			if f.expireAt == 0 || f.expireAt > ytime.TimestampMilli() {
				return f.value, nil
...
func (p *Pool) checkExpireItems() {
	...
	var timestampMilli = ytime.TimestampMilli()
	for {
