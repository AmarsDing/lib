...
type Ring struct {
	...
	len   *ytype.Int  // Length(already used size).
	cap   *ytype.Int  // Capability(>=len).
	dirty *ytype.Bool // Dirty, which means the len and cap should be recalculated. It's marked dirty when the size of ring changes.
}
