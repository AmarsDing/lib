...
func Valid(data interface{}) bool {
	return json.Valid(yconv.Bytes(data))
}
...
func Decode(data interface{}) (interface{}, error) {
	var value interface{}
	if err := DecodeTo(yconv.Bytes(data), &value); err != nil {
		return nil, err
	} else {
		return value, nil
	}
...
func DecodeTo(data interface{}, v interface{}) error {
	decoder := json.NewDecoder(bytes.NewReader(yconv.Bytes(data)))
...
func DecodeToJson(data interface{}, safe ...bool) (*Json, error) {
	if v, err := Decode(yconv.Bytes(data)); err != nil {
		return nil, err
	} else {
